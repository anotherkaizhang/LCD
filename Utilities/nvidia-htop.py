...
#                                    otherwise print first 120 characters.
...
parser.add_argument('-l', '--command-length', default=100, const=150, type=int, nargs='?')
...
#print(str(["ps", "-o", ps_format, "-p", ",".join(pid)]))
...
for line in processes.stdout.decode().split("\n"):
    ...
    command[idx] = parts[5][0:150]
    dquery ="./pid2docker.sh "+ str(pid[idx])
    dockerName[idx]= subprocess.run(dquery, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE).stdout.decode().strip()
#    print(dockerName[idx],pid[idx])
	
format = ("|  %3s  %8s   %25s   %8s  %8s %5s %5s %9s  %-" + str(command_length) + "." + str(command_length) + "s  |")

print(format % (
    "GPU", "PID", "DOCKERNAME",  "USER", "GPU MEM", "%CPU", "%MEM", "TIME", "COMMAND"
))
